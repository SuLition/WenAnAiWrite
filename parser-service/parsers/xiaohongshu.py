    ...
    def get_video_info(self) -> Optional[dict]:
        ...
        try:
            ...
            if not first_note_id:
                print(
                    f"[XHS] firstNoteId not found. Data keys: {list(self.data.keys())}"
                )
            ...
            for tag in tag_list:
                ...
                if tag_name:
                    hashtags.append({"id": tag.get("id", ""), "name": tag_name})
            ...
            if time_val:
                try:
                    create_time = datetime.fromtimestamp(time_val / 1000).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                except:
                    pass
